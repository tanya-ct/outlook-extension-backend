...
from app.services.ms_graph import fetch_email_by_id, fetch_all_emails
from app.services.email_parser import save_email_if_not_exists
...
async def save_email(
    ...
    try:
        ...
        result = save_email_if_not_exists(email_data, db)
        return result
...
async def fetch_emails(Authorization: str = Header(...), db: Session = Depends(get_db)):
    ...
    try:
        saved_count = await fetch_all_emails(access_token, db)
        return {"message": f"Saved {saved_count} emails"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
